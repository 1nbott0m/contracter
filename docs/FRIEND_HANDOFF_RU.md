...
Актуальное состояние ветки: Steam OpenID уже реализован на backend через
серверную проверку `check_authentication`, привязку `steam_id` и создание
сессии. Перед выпуском всё равно проверьте callback на Render после деплоя;
запрос без параметров должен вернуть `401`, а не `404`. Админские dashboard,
пользователи и audit требуют подтверждённую TOTP-сессию Google Authenticator.

Пополнения CC за реальные деньги в текущем MVP не реализованы. Не начисляйте
баланс напрямую из frontend или SQL без утверждённого платёжного webhook и
идемпотентного серверного зачисления.
