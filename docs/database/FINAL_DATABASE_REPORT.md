...
| 4–10 contract inputs | PENDING POSTGRESQL VERIFICATION | `economy_core::validate_inputs` (`MIN_INPUT_COUNT == 4`, `MAX_INPUT_COUNT == 10`); `finalize_contract` re-validates cardinality/uniqueness before any lock (0004, upgraded in 0022) | `economy-core/tests/tradeup.rs::rejects_input_counts_outside_the_four_to_ten_range`; `001_invariants.sql` 3/4/11-input checks | invariants doc row 3 | Run `db/verify.sh` against an isolated PostgreSQL database after applying migration 0022 |
...
| Collection weighting | PARTIAL | `build_outcomes`: exact `P(output) = input_count/total_input_count/output_count` | 5 dedicated tests, all passing | `economy-core/tests/tradeup.rs` | Never called from a real write path (§3) |
