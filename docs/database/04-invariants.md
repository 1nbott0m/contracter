...
| 3 | A contract consumes exactly 10 unique inputs of one rarity | none directly on `contract_inputs` row count (checked procedurally) | `economy_core::tradeup::validate_inputs` (`INPUT_COUNT == 10`, single rarity) | `finalize_contract` re-validates cardinality/uniqueness *before* any lookup or lock (0004) | `economy-core/tests/tradeup.rs::rejects_any_input_count_other_than_ten`, `rejects_mixed_input_rarities`; `db/tests/001_invariants.sql`: 9- and 11-input rejection |
