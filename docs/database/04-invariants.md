...
| 3 | A contract consumes 4 to 10 unique inputs of one rarity | none directly on `contract_inputs` row count (checked procedurally) | `economy_core::tradeup::validate_inputs` (`INPUT_RANGE = 4..=10`, single rarity, all distinct) | `finalize_contract` re-validates cardinality/uniqueness *before* any lookup or lock (0004, range relaxed by 0018) | `economy-core/tests/tradeup.rs::rejects_an_input_count_outside_four_to_ten`, `rejects_mixed_input_rarities`, `the_same_item_cannot_be_used_twice`; `db/tests/001_invariants.sql`: 3- and 11-input rejection, duplicate-input rejection, 4- and 10-input admission (0018) |
...
| 6 | A journal/history table cannot be written by direct `INSERT` from anything but its owning `SECURITY DEFINER` function | `require_journal_owner_insert()` trigger comparing `current_user` to the table owner, **or by name to `contracter_definer`** (0005, updated 0021) | n/a — DB-level | statement-level `BEFORE INSERT` trigger | not directly asserted by an executable test today (only indirectly, since every writer test goes through the function); **gap:** no test attempts a direct `INSERT` as `contracter_runtime` and asserts rejection |
