...
function App() {
  ...
  return <div className="app-shell">
    ...
    </main><footer><div><strong>CONTRACTER</strong><span>Контракты · Маркет · Инвентарь · История</span></div><div><strong>ИНФОРМАЦИЯ</strong><span>Прозрачность · Правила · Проверка операции</span></div><div><strong>18+</strong><span>Независимый сервис, не аффилированный с Valve Corporation или Steam.</span></div></footer>
  {revealOpen && <div className="reveal-backdrop"><div className={`reveal-panel ${processing ? 'is-processing' : 'is-result'}`}><span className="eyebrow">CONTRACTER / {processing ? 'LOCK · SIGN · MERGE' : 'RESULT'}</span>{processing ? <><h2>ФИКСИРУЕМ КОНТРАК</h2><div className="reveal-items">{selected.map((item) => <span key={item.id}>{item.weapon}<i /></span>)}</div><p>Выбранные предметы объединяются в один результат.</p></> : <><h2>КОНТРАК ЗАВЕРШЁН</h2><div className="result-skin"><b>AK-47</b><strong>WILD LOTUS</strong><span>Результат доступен после подтверждения API</span></div><button className="primary" onClick={() => { setRevealOpen(false); setSelected([]); }}>СОБРАТЬ НОВЫЙ <ArrowRight size={16} /></button><button className="close-login" onClick={() => setRevealOpen(false)}>ЗАКРЫТЬ</button></>}</div></div>}
