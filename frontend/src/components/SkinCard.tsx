...
export function SkinCard({ item, selected = false, onRemove, onAdd, onClick }: SkinCardProps) {
  ...
  return (
    <article className={`skin-card ${selected ? 'selected' : ''}`} onClick={handleClick}>
      <div className="skin-art" style={{ '--accent': item.color } as React.CSSProperties}>
        <SkinImage src={item.canonicalImageUrl || item.image} alt={`${item.weapon} | ${item.skin}`} accent={item.color} />
