...
export function MarketPage({ session, navigate, client = api, onBalanceChange, setApiStatus }: MarketPageProps) {
  ...
  return <>
    ...
    <div className="commerce-filters" aria-label="Фильтры маркета">
      ...
      <label>Редкость<select value={rarity} onChange={(event) => setRarity(event.target.value)}><option value="">Все</option>{rarities.map((value) => <option key={value}>{value}</option>)}</select></label>
