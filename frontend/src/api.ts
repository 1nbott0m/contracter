...
export type ApiInventoryItem = { public_id: string; sku_id: number; display_name: string; wear: string; value_microcredits: number; state: string };
...
/** Convert the wire representation into the shared view model when artwork is available. */
export function inventoryItemToSkin(item: ApiInventoryItem, image?: string | null): SkinInventoryItem {
  return {
    id: item.public_id,
    publicId: item.public_id,
    skuId: item.sku_id,
    weapon: item.display_name,
    skin: item.display_name,
    wear: item.wear,
    price: item.value_microcredits / 1_000_000,
    color: '#58d6e7',
    rarity: 'UNKNOWN',
    image: image || '',
