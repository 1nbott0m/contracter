...
/// Carried over from the parallel line of development on `main`, with the
/// four inputs made distinct: a contract may not consume one item twice.
#[test]
fn four_inputs_normalize_weights_by_the_actual_count() {
    let inputs = (0..4)
        .map(|_| input("in", "a", 2, dec!(0.20)))
        .collect::<Vec<_>>();
    let outcomes = build_outcomes(
...
#[test]
fn a_mixed_four_input_float_is_the_mean_of_the_actual_inputs() {
    let mut inputs = (0..3)
        .map(|_| input("low", "a", 2, dec!(0.10)))
        .collect::<Vec<_>>();
