...
BEGIN
    ...
    -- ALTER FUNCTION ... OWNER TO needs the migrating role to be able to
    -- become the new owner. Failing here, with the reason, is deliberate:
    -- carrying on would leave definer functions running as whoever owns
    -- them, which is the exposure this migration exists to remove.
    IF NOT pg_has_role(current_user, 'contracter_definer', 'MEMBER')
       AND NOT (SELECT rolsuper FROM pg_roles WHERE rolname = current_user) THEN
        RAISE EXCEPTION 'role % must be a member of contracter_definer (GRANT contracter_definer TO %) or a superuser to reassign SECURITY DEFINER ownership',
            current_user, current_user;
    END IF;

    GRANT USAGE ON SCHEMA public TO contracter_definer;
    -- Required by ALTER FUNCTION OWNER for a non-superuser migrator, and
    -- withdrawn again below so the definer role keeps no way to create
...
-- The published 0017 view stays exactly as published: SQLx verifies its
-- checksum, and `db/verify.sh` replays every migration over the same
-- database, so 0017's CREATE OR REPLACE VIEW must still succeed after this
-- migration has run. Reshaping `owned_inventory` in place would make that
-- replay fail ("cannot change name of view column"). A separate view has
-- the same body minus the internal `id`, and is what the runtime role reads.
CREATE OR REPLACE VIEW runtime_owned_inventory
...
REVOKE ALL ON runtime_owned_inventory FROM PUBLIC;

DO $block$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'contracter_runtime') THEN
        GRANT SELECT ON runtime_owned_inventory TO contracter_runtime;
        -- The runtime role reads the reshaped view only. The original view
        -- also exposes the internal inventory_items.id.
        REVOKE SELECT ON owned_inventory FROM contracter_runtime;
    END IF;
END;
$block$;
