...
- `crates/db` — SQLx 0.9, PostgreSQL pool, встроенные миграции, health-check, транзакции и типизированные ID.
...
`db/verify.sh` применяет миграции и seed-файлы идемпотентно, затем выполняет SQL-инварианты и проверку справочных данных в транзакциях. После этого интеграционный Rust-тест повторно проверяет те же миграции через SQLx, health-check и rollback. Финальный `ROLLBACK` ожидаем: тестовые записи не остаются в базе.
